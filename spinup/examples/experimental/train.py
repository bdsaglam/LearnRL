...
def train(env,
    ...
    def update(values, log_probs, entropies, returns):
        # Super critical!!
        v_optimizer.zero_grad()
        pi_optimizer.zero_grad()

        # Compute value and policy losses
        loss_v, loss_pi = ac.compute_loss(values=values,
                                          log_probs=log_probs,
                                          entropies=entropies,
                                          returns=returns,
        ...
        logger.store(LossPi=loss_pi.detach().cpu().item(),
                     LogPi=log_probs.detach().cpu().numpy(),
                     Pi=log_probs.detach().cpu().exp().numpy(),
                     MeanEntropy=entropies.detach().cpu().mean().numpy())
    ...
    for epoch in range(epochs):
        ...
        if dones[-1]:
            ...
            with torch.no_grad():
                obs_tensor = torch.tensor(obs, dtype=torch.float32).unsqueeze(0).to(device)
                v, _ = ac.step(obs_tensor)
                next_value = v.cpu().item()

        batch_value = torch.cat(value_tensors, 0).to(device)
        batch_log_prob = torch.cat(log_prob_tensors, 0).to(device)
        batch_entropy = torch.cat(entropy_tensors, 0).to(device)
        ...
        update(batch_value, batch_log_prob, batch_entropy, batch_return)
        ...
        if epoch % log_every == 0:
            ...
            logger.log_tabular('Values', with_min_and_max=True)
