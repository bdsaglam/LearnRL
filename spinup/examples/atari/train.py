...
def make_model(env, model_kwargs):
    ...
    var_counts = tuple(nn_utils.count_vars(module) for module in [feature_extractor, actor_network, critic_network])
    print('\nNumber of parameters: \t fe: %d, \t actor: %d, \t critic: %d\n' % var_counts)
...
def train(env,
    ...
    fe_optimizer = Adam(ac.feature_extractor_parameters(), lr=lr)
    ...
    def update(features, log_probs, entropies, returns):
        ...
        with torch.no_grad():
            for p, p_targ in zip(ac.parameters(), ac_targ.parameters()):
                ...
                p_targ.detach().mul_(polyak)
                p_targ.detach().add_((1 - polyak) * p.detach())
    ...
    for epoch in range(epochs):
        ...
        if dones[-1]:
            ...
            with torch.no_grad():
                ...
                feature_tensor = ac_targ.feature_extractor(batch_input).squeeze(0)
