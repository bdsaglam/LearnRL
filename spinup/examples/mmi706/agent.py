...
from spinup.core.bellman import calculate_returns, generalized_advantage_estimate
from spinup.examples.mmi706.networks import VisionModule, PathIntegrationModule, ActorCriticModule
...
class Agent(nn.Module):
    ...
    def send_context_to(self, device):
        self.previous_action_embedding = self.previous_action_embedding.to(device)
    ...
    def step(self, obs_tensor, previous_reward_tensor, goal_grid_code_tensor):
        ...
        visual_feature_map = self.vision_module(obs_tensor)
        ...
        self.current_grid_code, (self.pim_hx, self.pim_cx) = self.path_integration_module(visual_feature_map,
                                                                                          action_embedding,
                                                                                          (self.pim_hx, self.pim_cx))
    ...
    def predict_value(self, obs_tensor, previous_reward_tensor, goal_grid_code_tensor, context):
        # all tensors must be provided in batches
        if goal_grid_code_tensor is None:
            goal_grid_code_tensor = torch.zeros_like(self.current_grid_code)
        ...
        previous_action_embedding, current_grid_code, (pim_hx, pim_cx), (ac_hx, ac_cx) = context
    ...
    def compute_loss(self,
                     ...
                     policy_loss_coef=1,
                     entropy_reg_coef=1,
                     grid_layer_weight_reg_loss_coef=1
                     ):
        device = self.get_device()
        ...
        loss_grid_l2 = self.path_integration_module.l2_loss() * grid_layer_weight_reg_loss_coef

        # total loss
        loss = loss_v + loss_pi + loss_entropy + loss_grid_l2
    ...
    def act(self, obs, previous_reward, goal_grid_code, deterministic=False):
        ...
        previous_reward = torch.tensor([previous_reward]).float().unsqueeze(0).to(device)
...
def make_agent(env,
    ...
    vision_module = VisionModule(input_shape=image_shape)
    path_integration_module = PathIntegrationModule(
    ...
    print(f'Path integration module: \t {nn_utils.count_vars(path_integration_module):d}')
    print(f'Actor critic module: \t {nn_utils.count_vars(actor_critic_module):d}')
